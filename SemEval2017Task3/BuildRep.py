...
class BuildRep:
	...
	def BuildTrainRep(self,question_comments,truth_table,oversample=False,ovsize=4,out_path='.'):
		"""
		...
		"""
		with open(join_path(out_path,'trainov.csv'),'w',encoding='utf=8') as out:
			...
			for question in question_comments.keys():
				...
				for id,comment in question_comments[question]:
					...
					if oversample == True and truth_table[id] == 'true':
						extra_questions = self.GetOverSampledText(question[-1],ovsize)
						extra_comments  = self.GetOverSampledText(comment,ovsize)
						for extra_question,extra_comment in zip(extra_questions,extra_comments):
							exq_embedding = self.GetSentenceEmbedding(extra_question)
							exc_embedding = self.GetSentenceEmbedding(extra_comment)
							if exc_embedding.__class__ == np.ndarray and exq_embedding.__class__ == np.ndarray:
								exqc_embedding = np.concatenate((exq_embedding,exc_embedding))
								if not self.dot_product:
									out.write(','.join(str(v) for v in exqc_embedding)+','+truth_table[id])
								else:
									dp = np.dot(exq_embedding,exc_embedding)
									out.write(','.join(str(v) for v in exqc_embedding)+','+str(dp)+','+truth_table[id])
								out.write('\n')
	...
	def GetOverSampledText(self,text,ovsize):
		words = word_tokenize(text)
		candidates = np.zeros((len(words),ovsize),dtype=str)
		for x in range(len(words)):
			try:
				candidates[x] =np.asarray( [ w[0] for w in self.embedding_model.similar_by_word(words[x],topn=ovsize)])
			except KeyError:
				candidates[x] = np.asarray([words[x] for _ in range(ovsize)])
				print('oov word : ',words[x])
		ovtext = []
